...
/*   Updated: 2024/09/10 23:24:13 by atrabut          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*create_token_parsing(t_data *parsing, t_token	*new_token)
{
	...
	new_token->str = (char *)malloc(sizeof(char)
			* (parsing->i + 2 - parsing->last_tok));
	if (!new_token->str)
		return (ft_putstr_fd("error malloc create_token\n", 2),
			free(new_token), NULL);
	return (new_token);
...
void	new_token_init(t_data *parsing, t_token *new_token, int i)
{
	new_token->str[i] = '\0';
...
int	create_token(t_data *parsing)
{
	...
	{
		if ((index != parsing->i && index != parsing->last_tok)
			|| !is_blank(parsing->str[index]))
			new_token->str[i++] = parsing->str[index];
		index++;
	}
	new_token_init(parsing, new_token, i);
	return (1);
}
